...
export class EditarComponent implements OnInit {
  constructor(private activerouter:ActivatedRoute, private router:Router, private api:EstudiantesService) { }
...
datosEstudiante: any;
  ...
  postForm(form:any){
    let estudianteid = this.activerouter.snapshot.paramMap.get('id');
    this.api.putEstudiante(form, estudianteid).subscribe(data => this.router.navigate(['estudiantes/listar']))
...
eliminar(){
  let estudianteid = this.activerouter.snapshot.paramMap.get('id');
  this.api.deleteEstudiante(estudianteid).subscribe(data => this.router.navigate(['estudiantes/listar']))
