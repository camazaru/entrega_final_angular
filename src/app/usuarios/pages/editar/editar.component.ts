...
export class EditarComponent {
  ...
  constructor(private activerouter:ActivatedRoute, private router:Router, private api:UserService) { }
...
datosUsuario: any;
  ...
  postForm(form:any){
    let usuarioid = this.activerouter.snapshot.paramMap.get('id');
    this.api.putUser(form, usuarioid).subscribe(data => this.router.navigate(['usuarios/listar']))
...
eliminar(){
  let usuarioid = this.activerouter.snapshot.paramMap.get('id');
  this.api.deleteUser(usuarioid).subscribe(data => this.router.navigate(['usuarios/listar']))
