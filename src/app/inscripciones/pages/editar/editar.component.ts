...
export class EditarComponent implements OnInit {
  ...
  datosInscripcion: any;
    ...
    postForm(form:any){
      let inscripcionid = this.activerouter.snapshot.paramMap.get('id');
      this.api.putInscripcion(form, inscripcionid).subscribe(data => {this.router.navigate(['inscripciones/listar'])})
  ...
  eliminar(){
    let inscripcionid = this.activerouter.snapshot.paramMap.get('id');
    this.api.delete(inscripcionid).subscribe(data => {this.router.navigate(['inscripciones/listar'])})
